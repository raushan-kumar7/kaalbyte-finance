...
const StackLayout = () => {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        // ✅ Never clear turso_migrated here — that's managed by SyncService
        // ✅ Never clear RESTORE_DONE_KEY here — that's cleared by AuthService per login
          ...
          try {
            if (user) {
              ...
              if (profile) {
                ...
                const didRestore = await maybeRestoreFromCloud(user.uid);
                if (didRestore) {
                  // ✅ Re-mount entire Stack so all screens re-query local SQLite
                  setRestoreKey((k) => k + 1);
                }
              } else {
                clearSession();
  ...
  return (
    <AuthGuard isInitializing={isInitializing}>
      {/*
        key={restoreKey} forces full Stack re-mount after cloud restore
        so every screen re-queries local SQLite and shows restored data
      */}
      <Stack key={restoreKey} screenOptions={{ headerShown: false }}>
        <Stack.Screen name="index" />
        <Stack.Screen name="(auth)" options={{ gestureEnabled: false }} />
...
async function maybeRestoreFromCloud(userId: string): Promise<boolean> {
  try {
    ...
    // ✅ Mark done after success — prevents re-running on hot reload / re-mount
    await AsyncStorage.setItem(RESTORE_DONE_KEY, "true");

    if (didRestore) {
      console.log(
        "[Layout] ✅ Tables restored from Turso — triggering re-render",
      );
    } else {
      console.log(
        "[Layout] ✅ Smart sync complete — local was already up to date",
      );
    }
    ...
    console.error("[Layout] ❌ maybeRestoreFromCloud failed:", err);
    // ✅ Do NOT set key on failure — allows retry on next login
    return false;
  }
}
