...
// const [lastSyncTime, setLastSyncTime] = useState<string>(syncTime);
...
//     await SyncService.smartSync(user.uid);

//     // After cloud sync, refresh local queries
//     onRefresh();
...
//     );
//   } catch (err) {
//     console.error("[Assets] Manual sync failed:", err);
//   } finally {
//     setIsSyncing(false);
//   }
// }, [isSyncing, onRefresh]);
      ...
      <Animated.View entering={FadeInDown.duration(400)} style={{ paddingTop: 32, paddingHorizontal: 4, marginBottom: 16 }}>
        ...
        >
          ...
          <View style={{ alignItems: "flex-end", gap: 6 }}>
            ...
            >
              {isSyncing ? (
                <ActivityIndicator size={8} color={colors.gold[500]} />
              ) : (
                <View
                  style={{
                    width: 5, height: 5, borderRadius: 3,
              ...
              <Typo
                className="font-mono text-[9px] uppercase tracking-wider"
                style={{ color: isSyncing ? colors.gold[500] : colors.success[500] }}
              >
            ...
            >
              <RefreshCw size={10} color={isSyncing ? colors.gold[500] : colors.text.muted} />
              <Typo
                className="font-mono text-[9px]"
                style={{ color: isSyncing ? colors.gold[500] : colors.text.muted }}
              >
                {lastSyncTime}
              </Typo>
            </TouchableOpacity>
