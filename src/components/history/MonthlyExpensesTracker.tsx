...
import { TrendingDown, TrendingUp, Wallet, LayoutList, PieChart, Receipt } from "lucide-react-native";
...
export interface MonthlyExpensesTrackerProps {
  data: {
    ...
    grandTotal: number;
    /**
     * Each entry needs both `rawDate` (ISO string for the edit form)
     * and `date` (formatted display string for the table rows).
     * Map from your DB rows: rawDate = e.date, date = formatted display.
     */
    entries?: ExpenseRow[];
  };
  totalIncome?: number;
...
const MonthlyExpensesTracker = ({
  ...
  const entryCounts = (data.entries ?? []).reduce<Record<string, number>>((acc, e) => {
    ...
    >
      ...
      <View className="px-6 pt-8 pb-4">
        <View className="flex-row items-center justify-between">
          <View>
            <Typo variant="muted" className="font-mono text-[10px] uppercase tracking-widest mb-1">
              Expense Report
            </Typo>
            <Typo className="text-white font-serif-bold text-3xl">{data.month}</Typo>
          </View>
          <View className="bg-brand-800 px-4 py-2 rounded-2xl border border-white/5">
            ...
            <Typo className="text-white font-sans-medium text-sm mt-0.5">{data.month_range}</Typo>
          </View>
        </View>
      </View>

      {/* ── Summary Card ─────────────────────────────────────────────── */}
      <Animated.View entering={FadeInDown.delay(100).duration(400)} className="mx-6 mb-5">
          ...
          {totalIncome > 0 && (
            <View className="pt-4 border-t border-white/5">
              <View className="flex-row justify-between items-center mb-3">
                <View>
                  <Typo variant="muted" className="text-xs font-mono mb-1">Income</Typo>
                ...
                <View className="items-end">
                  ...
                  <Typo
                    ...
                    style={{ color: isOverBudget ? colors.danger[500] : colors.success[500] }}
                  >
      ...
      {/* ── Tab Switcher ─────────────────────────────────────────────── */}
      <Animated.View entering={FadeInDown.delay(160).duration(350)} className="mx-6 mb-5">
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  size={13}
                  color={isActive ? colors.white : colors.text?.muted ?? "#888"}
                />
                <Typo
        ...
        {activeTab === "summary" && (
          <>
            ...
            <View className="px-6 mb-6">
              <Typo variant="muted" className="font-mono text-[10px] uppercase tracking-widest mb-4">
                Budget Allocation
              </Typo>
                  ...
                  return (
                    ...
                    >
                      <View className="bg-brand-800 rounded-[24px] p-4 border border-white/5">
                        ...
                        <Typo variant="muted" className="font-mono text-[9px] uppercase tracking-wider mb-1">
                          {bucket.label}
                        </Typo>
                        ...
                        <View className="flex-row items-center">
                          <Typo className="font-mono text-xs" style={{ color: bucket.color }}>
                            {percentage}%
                          </Typo>
