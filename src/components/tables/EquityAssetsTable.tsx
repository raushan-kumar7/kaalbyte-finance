...
export interface EquityAssetsTableProps {
  ...
  onUpdate: (
    id: number,
    updates: Partial<
      Omit<EquityAsset, "id" | "userId" | "createdAt" | "updatedAt">
    >,
  ) => Promise<boolean>;
...
const EditEquityForm = ({
  ...
  const totalPreview =
    (Number(form.pricePerShare) || 0) * (Number(form.totalShares) || 0);
  ...
  return (
    <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          {Object.values(Exchange).map((ex) => (
            <ExchangeTag
              ...
              onPress={() =>
                !isSaving && setForm((p) => ({ ...p, exchange: ex }))
              }
            />
          ))}
        </View>
      ...
      <View
        style={{
          backgroundColor: `${colors.success[500]}0D`,
          borderRadius: 14,
          borderWidth: 1,
...
const EquityRow = ({
  ...
  <Animated.View entering={FadeInDown.delay(index * 55).duration(320)}>
    ...
    >
      ...
      {/* Company */}
      <Typo className="text-white font-sans-bold text-base mb-3">
        {item.company}
      </Typo>
      ...
      <View style={{ flexDirection: "row", gap: 8 }}>
        {[
          {
            label: "Price/share",
            value: `₹${(item.pricePerShare ?? 0).toLocaleString("en-IN")}`,
            accent: false,
          },
          {
            label: "Shares",
            value: String(item.totalShares ?? 0),
            accent: false,
          },
          {
            label: "Invested",
            value: `₹${(item.totalAmount ?? 0).toLocaleString("en-IN")}`,
            accent: true,
          },
          ...
          >
            ...
            <Typo
              ...
              style={{
                color: accent ? colors.success[500] : colors.text.primary,
              }}
            >
              {value}
            </Typo>
...
const EquityAssetsTable = ({
  assets,
  isLoading = false,
  onUpdate,
  onDelete,
}: EquityAssetsTableProps) => {
  const [editTarget, setEditTarget] = useState<EquityAsset | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<EquityAsset | null>(null);
    ...
    return (
      ...
      >
        ...
        <Typo className="text-white font-sans-medium text-sm mb-1">
          No equity assets yet
        </Typo>
        <Typo
          className="font-mono text-[11px]"
          style={{ color: colors.text.muted }}
        >
          Add stock or mutual fund holdings
        </Typo>
      </Animated.View>
    );
  ...
  return (
    <>
      ...
      <ConfirmDeleteModal
        ...
        amount={deleteTarget?.totalAmount ?? 0}
        description={deleteTarget?.company ?? ""}
        category={deleteTarget?.exchange ?? ""}
